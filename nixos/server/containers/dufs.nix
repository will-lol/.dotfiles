{ pkgs, config, ... }: {
  ...
  systemd.services."podman-dufs-secrets" = {
    ...
    script = ''
      ...
      printf "@/pub:ro|admin:$(cat ${
        config.sops.secrets."dufs/hash".path
      })@/:rw,/pub:rw" | ${pkgs.podman}/bin/podman secret create dufs-auth -
    '';
  };
  ...
  virtualisation.oci-containers.containers = {
    dufs = {
      extraOptions = [ "--secret=dufs-auth,type=env,target=DUFS_AUTH" ];
      ports = [ "80:5000" ];
      volumes = [ "/mnt/store:/data" ];
      image = "dufs";
      autoStart = true;
