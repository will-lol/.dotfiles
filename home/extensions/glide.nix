...
in
{
  options = {
    services.glide-wm = {
      ...
      package = lib.mkPackageOption pkgs "glide" { };
      ...
      launchd = {
        enable = lib.mkOption {
          ...
          default = true;
          description = ''
            Configure the launchd agent to manage the glide process.
            ...
            Run: `launchctl list | grep glide`

            - A running process will show a Process ID (PID) and a status of 0, for example:
              `12345	0	org.nix-community.home.glide`

            - If the service has crashed or failed to start, the PID will be a dash and the
              status will be a non-zero number, for example:
              `-	1	org.nix-community.home.glide`

            In case of failure, check the logs with `cat /tmp/glide.err.log`.

            For more detailed service status, run `launchctl print gui/$(id -u)/org.nix-community.home.glide`.
          '';
        };
